...
#define CLIENT_PORT_NUM 6000
#define CLIENT_IP_ADDRESS "192.168.1.43"
...
void Task_TXRX(void)
{
	...
	*/
	...
	{
	printf("Bind to Port Number %d ,IP address %s failed\n", CLIENT_PORT_NUM, CLIENT_IP_ADDRESS);
	close(socket_fd);
	exit(1);
	}
	...
	int accept_fd = accept(socket_fd, (struct sockaddr*)&isa,&addr_size);
	if(accept_fd < 0)
	{
...
int mount_sd()
{
	...
	g_Drive[1] = ':';
	g_Drive[2] = '\0';
	if (f_mount(&g_FileSys, g_Drive, 1)) {
...
void Task_MTL(void)
{
	...
	for(int i=4;i>0;i--)
	{
		sprintf(str,"Screen demo will launch in %d",i-1);
		vid_clean_screen(pReader, BLACK_24);
